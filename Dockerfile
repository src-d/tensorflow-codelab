...
ADD model2tb /usr/local/bin/

RUN apt-get update && \
    apt-get install -y --no-install-suggests --no-install-recommends ca-certificates curl locales git libc6-dev gcc python3 nano && \
...
RUN curl https://dl.google.com/go/go1.10.1.linux-amd64.tar.gz | tar -C /usr/local -xz && \
    curl -L "https://storage.googleapis.com/tensorflow/libtensorflow/libtensorflow-cpu-linux-x86_64-1.8.0.tar.gz" | tar -C /usr/local -xz && \
    ldconfig && \
